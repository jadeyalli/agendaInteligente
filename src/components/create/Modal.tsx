...
/** =========================================================
 * CreateEditModal
 * - Modal minimalista con pestañas: Evento / Tarea / Solicitud
 * - Reutilizable para CREAR o EDITAR
 * - No hace llamadas a red: delega submit al padre (onSubmit)
 * - Header fijo, body con scroll, tamaño reducido, íconos
 * ========================================================= */
...
type Props = {
    open: boolean;
    mode?: "create" | "edit";
    initialTab?: "evento" | "tarea" | "solicitud";
    initialValues?: Partial<EventForm & TaskForm & RequestForm>;
    /** título del modal (por defecto: Crear / Editar) */
    title?: string;
            ...
            >
                ...
                <motion.div
                    ...
                    className={classNames(
                        card,
                        // Tamaño reducido + layout para header sticky y body scroll
...
// Field ahora acepta íconos para etiqueta y para el campo
// Tip auxiliar: un elemento que acepta className
...
const Tabs: React.FC<{ tabs: { id: string; label: string }[]; value: string; onChange: (v: string) => void }> = ({
    tabs,
    value,
    onChange,
}) => (
...
const defaultTask = (): TaskForm => ({ kind: "TAREA", title: "", description: "", category: "", repeat: "NONE", dueDate: "" });
...
function mapEvent(f: EventForm) {
    ...
    if (isOpcional) {
        return { ...base, status: "WAITLIST", participatesInScheduling: false, repeat: f.repeat };
    }
    if (isCritica) {
        return {
            ...base,
            ...
            transparency: "OPAQUE",
            start: f.date && f.timeStart ? new Date(`${f.date}T${f.timeStart}:00Z`) : null,
            end: f.date && f.timeEnd ? new Date(`${f.date}T${f.timeEnd}:00Z`) : null,
            repeat: f.repeat,
        };
    }
    // URGENTE / RELEVANTE
    return {
        ...
        transparency: "OPAQUE",
        window: f.window,
        windowStart: f.window === "RANGO" && f.windowStart ? new Date(`${f.windowStart}T00:00:00Z`) : null,
        windowEnd: f.window === "RANGO" && f.windowEnd ? new Date(`${f.windowEnd}T23:59:59Z`) : null,
        repeat: f.repeat,
        start: f.date && f.timeStart ? new Date(`${f.date}T${f.timeStart}:00Z`) : null,
        end: f.date && f.timeEnd ? new Date(`${f.date}T${f.timeEnd}:00Z`) : null,
    };
}
function mapTask(f: TaskForm) {
    return {
        kind: "TAREA",
        title: f.title.trim(),
        description: f.description?.trim() || null,
        category: f.category || null,
        repeat: f.repeat,
        dueDate: f.dueDate ? new Date(`${f.dueDate}T23:59:59Z`) : null,
    };
}
...
const CrearTarea: React.FC<{ initial?: Partial<TaskForm>; onSubmit: (data: any) => void }> = ({ initial, onSubmit }) => {
    const initTask = useMemo<TaskForm>(() => ({ ...defaultTask(), ...(initial ?? {}) }), [initial]);
    const [f, set] = useState<TaskForm>(initTask);
    const canSubmit = f.title.trim().length > 0;
    ...
    return (
        <div className="space-y-4">
            ...
            <Row>
                <Field label="Fecha límite (opcional)" labelIcon={<Calendar className="h-4 w-4" />}>
                    <Input type="date" value={f.dueDate} onChange={(e) => set({ ...f, dueDate: e.target.value })} />
                </Field>
                <Field label="Repetición" labelIcon={<RepeatIcon className="h-4 w-4" />}>
                    <Select value={f.repeat} onChange={(e) => set({ ...f, repeat: e.target.value as RepeatRule })}>
                        <option value="NONE">No repetir</option>
            ...
            <div className="flex justify-end gap-2 pt-2">
                <button className={button} type="button" onClick={() => set(initTask)}>
                    Limpiar
                </button>
                <button
                    className={classNames(primary, !canSubmit && "opacity-50 cursor-not-allowed")}
                    disabled={!canSubmit}
                    onClick={() => onSubmit(mapTask(f))}
                    type="button"
                >
                    Guardar
                </button>
...
export default function CreateEditModal({
    ...
    const [tab, setTab] = useState<"evento" | "tarea" | "solicitud">(initialTab);
        ...
        <ModalShell open={open} onClose={onClose} title={derivedTitle}>
            <Tabs
                tabs={[
                    ...
                    { id: "tarea", label: mode === "edit" ? "Editar Tarea" : "Crear Tarea" },
